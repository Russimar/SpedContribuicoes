...
type
  ...
  TEnumIND_REG_CUM = (tpRegimeCaixa, tpRegimeCompetEscritConsolidada,
    tpRegimeCompetEscritDetalhada);
...
begin
  ...
  begin
    ...
    SUFRAMA := '';
    case AnsiIndexStr(qryEmpresaIND_NATUREZA_PJ.AsString,
      ['00', '01', '02', '03', '04', '05']) of
      0:
...
begin
  ...
  begin
    case Ind_Incidencia of
      tpEscrOpIncNaoCumulativo:
        COD_INC_TRIB := codEscrOpIncNaoCumulativo;
      tpEscrOpIncCumulativo:
    ...
    case Ind_Credito of
      ...
      tpRegimeCompetEscritConsolidada:
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        case AnsiIndexStr(sqlProdutoTIPO_ITEM.AsString, ['00', '01', '02', '03', '04',
          '05', '06', '07', '08', '09', '10', '99']) of
          0:
            TIPO_ITEM := tiMercadoriaRevenda; // 00 – Mercadoria para Revenda
          1:
            TIPO_ITEM := tiMateriaPrima; // 01 – Matéria-Prima;
          2:
            TIPO_ITEM := tiEmbalagem; // 02 – Embalagem;
          3:
            TIPO_ITEM := tiProdutoProcesso; // 03 – Produto em Processo;
          4:
            TIPO_ITEM := tiProdutoAcabado; // 04 – Produto Acabado;
          5:
            TIPO_ITEM := tiSubproduto; // 05 – Subproduto;
          6:
            TIPO_ITEM := tiProdutoIntermediario; // 06 – Produto Intermediário;
          7:
            TIPO_ITEM := tiMaterialConsumo; // 07 – Material de Uso e Consumo;
          8:
            TIPO_ITEM := tiAtivoImobilizado; // 08 – Ativo Imobilizado;
          9:
            TIPO_ITEM := tiServicos; // 09 – Serviços;
          10:
            TIPO_ITEM := tiOutrosInsumos; // 10 – Outros Insumos;
          11:
            TIPO_ITEM := tiOutras; // 99 – Outras
        end;
...
{$REGION 'Cupom Fiscal'}
        ...
        begin
          ...
          begin
            ...
            VL_DOC := sqlConsulta.FieldByName('CUPON2TOTITENS').AsFloat -
              sqlConsulta.FieldByName('CUPON2DESC').AsFloat;
            ...
            sqlConsulta2.SQL.Add
              ('select sum(CI.TOTAL_ITEM * (coalesce(P.PRODN2ALIQPIS, 0) / 100)) VALOR_PIS, ');
            sqlConsulta2.SQL.Add
              ('sum(CI.TOTAL_ITEM * (coalesce(P.PRODN2ALIQCOFINS, 0) / 100)) VALOR_COFINS, ');
            sqlConsulta2.SQL.Add
              ('sum(CI.TOTAL_ITEM) TOTAL_ITEM, sum(coalesce(CI.CPITN2DESC, 0) + ');
            ...
            sqlConsulta2.SQL.Add
              ('sum(coalesce(CI.CPITN2VLRICMS, 0)) CPITN2VLRICMS from CUPOMITEM CI ');
            ...
            sqlC175.Close;
            sqlC175.ParamByName('ID_CUPOM').AsString :=
              sqlConsulta.FieldByName('cupoa13id').AsString;
            sqlC175.open;
            sqlC175.First;
...
{$REGION 'Nota Fiscal'}
        ...
        begin
          ...
          begin
            ...
            sqlConsulta2.SQL.Add('NOFIA13ID = ''' + sqlConsulta.FieldByName('NOFIA13ID')
              .AsString + ''' order by CTRCICOD');
            sqlConsulta2.open;
            NroDocumento := sqlConsulta.FieldByName('NOFIINUMERO').AsString;
            DataDocumento := FormatDateTime('ddmmyyyy',
              sqlConsulta.FieldByName('NOFIDEMIS').AsDateTime);
            ...
            else if sqlConsulta.FieldByName('NOFA1ENTRADASAIDA').AsString = 'E' then
            begin
              EntradaSaida := '0';
            end
            else
            ...
            else if sqlConsulta.FieldByName('NOFICFRETEPORCONTA').AsString = 'F' then
...
{$REGION 'Item Nota Fiscal'}
            ...
            sqlConsulta2.SQL.Clear;
            sqlConsulta2.SQL.Add
              ('Select P.PRODA60DESCR, P.PRODA2CSTIPI, P.PRODA3CSTIPIENTRADA, ');
            sqlConsulta2.SQL.Add
              ('P.PRODA2CSTPIS, P.PRODA3CSTPISENTRADA, P.PRODA2CSTCOFINS, ');
            sqlConsulta2.SQL.Add
              ('P.PRODA3CSTCOFINSENTRADA, P.PRODN2ALIQCOFINS, P.PRODN2ALIQPIS, ');
            sqlConsulta2.SQL.Add
              ('P.PRODN2ALIQCOFINS, P.PRODIORIGEM, U.UNIDA5DESCR, I.* ');
            ...
            sqlConsulta2.SQL.Add
              ('LEFT JOIN NOTAFISCAL NF ON NF.NOFIA13ID = I.NOFIA13ID ');
            sqlConsulta2.SQL.Add('Where I.NOFIA13ID=''' + sqlConsulta.FieldByName
              ('NOFIA13ID').AsString + '''');
            ...
            begin
              ...
              begin
                ...
                inc(ordem);
                ValorBase := sqlConsulta2.FieldByName('NFITN2VLRUNIT').AsFloat *
                  sqlConsulta2.FieldByName('NFITN3QUANT').AsFloat;
                ...
                if length(sqlConsulta2.FieldByName('NFITICST').Value) < 3 then
                  CST := sqlConsulta2.FieldByName('PRODIORIGEM').AsString +
                    FormatFloat('00', sqlConsulta2.FieldByName('NFITICST').Value)
                else
                ...
                if sqlConsulta2.FieldByName('NFITICST').AsInteger in ([30, 40, 41, 50, 60])
                then
                begin
                  VL_BC_ICMS := 0;
                  ALIQ_ICMS := 0;
                ...
                if (OpEntraSai = 'S') or
                  (sqlConsulta.FieldByName('NOFA1ENTRADASAIDA').AsString = 'S') then
                begin
                  if copy(sqlConsulta2.FieldByName('PRODA2CSTIPI').AsString, 0, 2) = ''
                  then
                    CST_IPI := StrToCstIpi('99')
                  else
                    CST_IPI :=
                      StrToCstIpi(copy(sqlConsulta2.FieldByName('PRODA2CSTIPI')
                      .AsString, 0, 2));
                end
                else
                begin
                  if copy(sqlConsulta2.FieldByName('PRODA3CSTIPIENTRADA').AsString, 0,
                    2) = '' then
                    CST_IPI := StrToCstIpi('99')
                  else
                    CST_IPI :=
                      StrToCstIpi(copy(sqlConsulta2.FieldByName('PRODA3CSTIPIENTRADA')
                      .AsString, 0, 2));
                end;
                COD_ENQ := '999';
                ...
                if (OpEntraSai = 'S') or
                  (sqlConsulta.FieldByName('NOFA1ENTRADASAIDA').AsString = 'S') then
                // operacao de estoque de Saida
                begin
                  if copy(sqlConsulta2.FieldByName('PRODA2CSTPIS').AsString, 0, 2) = ''
                  then
                  begin
                  ...
                  begin
                    CST_PIS :=
                      StrToCstPis(copy(sqlConsulta2.FieldByName('PRODA2CSTPIS')
                      .AsString, 0, 2));
                    CST_COFINS :=
                      StrToCstCofins(copy(sqlConsulta2.FieldByName('PRODA2CSTCOFINS')
                      .AsString, 0, 2));
                  end;
                end
                else
                begin
                  if copy(sqlConsulta2.FieldByName('PRODA3CSTPISENTRADA').AsString, 0,
                    2) = '' then
                  begin
                    CST_PIS := StrToCstPis('99');
                    CST_COFINS := StrToCstCofins('99');
                  ...
                  begin
                    CST_PIS :=
                      StrToCstPis(copy(sqlConsulta2.FieldByName('PRODA3CSTPISENTRADA')
                      .AsString, 0, 2));
                    CST_COFINS :=
                      StrToCstCofins
                      (copy(sqlConsulta2.FieldByName('PRODA3CSTCOFINSENTRADA')
                      .AsString, 0, 2));
                  end;
                end;
                VL_BC_PIS := ValorBase;
                ...
                // QUANT_BC_PIS := 0;
                // ALIQ_PIS_R := 0;
                VL_PIS := ValorBase * (sqlConsulta2.FieldByName('PRODN2ALIQPIS')
                  .Value / 100);
...
{$REGION 'Nota Compra'}
        ...
        begin
          ...
          begin
            sqlConsulta2.Close;
            sqlConsulta2.SQL.Text := 'Select * From CONTASPAGAR Where NOCPA13ID = ''' +
              sqlConsulta.FieldByName('NOCPA13ID').AsString + ''' order by CTPGICOD';
            sqlConsulta2.open;
            if sqlConsulta2.FieldByName('CTPGDVENC').isnull then
              IND_PGTO := tpVista
            else if (sqlConsulta2.FieldByName('CTPGDVENC')
              .AsString = sqlConsulta.FieldByName('NOCPDEMISSAO').AsString) then
              IND_PGTO := tpVista
            else
              IND_PGTO := tpPrazo;
            sqlConsulta2.Next;
            ...
            sqlSerie.ParamByName('ID_NOTA').AsString :=
              sqlConsulta.FieldByName('NOCPA13ID').AsString;
            sqlSerie.open;
            ...
            VL_ABAT_NT := 0;
            VL_MERC := sqlConsulta.FieldByName('TOTAL_MERC').AsFloat +
              sqlConsulta.FieldByName('VL_ICMS_ST').AsFloat;
            if sqlConsulta.FieldByName('NOCPA5TIPOFRETE').AsString = 'C' then
              IND_FRT := tfPorContaEmitente
            else if sqlConsulta.FieldByName('NOCPA5TIPOFRETE').AsString = 'F' then
              IND_FRT := tfPorContaDestinatario
            else
              IND_FRT := tfSemCobrancaFrete;
...
{$REGION 'Item Nota Compra'}
            ...
            begin
              ...
              begin
                NUM_ITEM := IntToStr(ordem);
                inc(ordem);
                ValorBase := (sqlConsulta2.FieldByName('NOCIN3VLREMBAL').Value *
                  sqlConsulta2.FieldByName('NOCIN3QTDEMBAL').Value) +
                ...
                if sqlConsulta2.FieldByName('NOCA3CSTIPI').AsString = '' then
                  ...
                  CST_IPI :=
                    StrToCstIpi(copy(sqlConsulta2.FieldByName('NOCA3CSTIPI')
                    .AsString, 0, 2));
                ...
                else if sqlConsulta2.FieldByName('PRODA3CSTPISENTRADA').AsString = '' then
                ...
                if sqlConsulta2.FieldByName('Noca3CSTCOFINS').AsString <> '' then
                  CST_COFINS :=
                    StrToCstCofins(sqlConsulta2.FieldByName('Noca3CSTCOFINS').AsString)
                else if sqlConsulta2.FieldByName('PRODA3CSTCOFINSENTRADA').AsString = ''
                then
                  CST_COFINS := StrToCstCofins('49')
                else
                  CST_COFINS :=
                    StrToCstCofins(copy(sqlConsulta2.FieldByName('PRODA3CSTCOFINSENTRADA')
                    .AsString, 0, 2));
...
begin
  ...
  begin
    ...
    sqlConsulta2.SQL.Clear;
    sqlConsulta2.SQL.Add
      ('select (NOCIN3VLREMBAL * NOCIN3QTDEMBAL) VALOR_TOTAL, NOCA3CSTCOFINS, ');
    sqlConsulta2.SQL.Add
      ('NOCA3CSTPIS, NOCIN2PERCCOFINS, NOCIN2PERCPIS, NOCIN2VLRCOFINS, NOCIN2VLRPIS, NOCIN2BASECOFINS, NOCIN2BASEPIS ');
    sqlConsulta2.SQL.Add('from NOTACOMPRAITEM where NOCPA13ID = ' +
      sqlConsulta.FieldByName('NOCPA13ID').AsString);
    sqlConsulta2.open;
    while not sqlConsulta2.Eof do
    begin
...
begin
  ...
  Aliq_Pis_Empresa := StrToFloat(SQLLocate('EMPRESA', 'EMPRICOD', 'PERC_PIS',
  ...
  sqlConsulta.SQL.Add
    ('sum(SP.VALOR_PIS) VALOR_PIS, SP.CST_PIS, SP.PERC_PIS, SP.COD_BASE_CALCULO ');
  ...
  sqlConsulta2.SQL.Clear;
  sqlConsulta2.SQL.Add
    (' select sum(SP.VALOR_PIS) VALOR_PIS, sum(SP.VALOR_BASE) VALOR_BASE, ');
  sqlConsulta2.SQL.Add(' iif (SP.CST_PIS = ' + QuotedStr('01') + ', ' + QuotedStr('01') +
    ', ' + QuotedStr('32') + ') as CST_PIS, SP.ALIQ_PIS ');
  sqlConsulta2.SQL.Add('from SPED_PISCOFINS_SAIDA SP ');
  sqlConsulta2.SQL.Add('WHERE SP.data_emissao between ' +
    QuotedStr(FormatDateTime('mm/dd/yyyy', DataInicial)) + ' AND ');
  ...
  begin
    ...
    Mem100VlrBasePis.AsFloat := Mem100VlrBasePis.AsFloat + sqlConsulta.FieldByName
      ('VALOR_BASE_PIS').AsFloat;
    Mem100AliqPis.AsFloat := sqlConsulta.FieldByName('PERC_PIS').AsFloat;
    Mem100VlrCredito.AsFloat := Mem100VlrCredito.AsFloat + sqlConsulta.FieldByName
      ('VALOR_PIS').AsFloat;
    Mem100VlrCreditoDisponivel.AsFloat := Mem100VlrCreditoDisponivel.AsFloat +
      sqlConsulta.FieldByName('VALOR_PIS').AsFloat;
    ...
    begin
      ...
      Mem100SaldoCredito.AsFloat := Mem100VlrCredito.AsFloat -
        Mem100VlrCreditoDescontado.AsFloat;
  ...
  begin
    ...
    begin
      ...
      sqlConsulta.Filter := 'PERC_PIS = ' +
        StringReplace(FormatFloat('0.00', Mem100AliqPis.AsFloat), ',', '.',
        [rfReplaceAll]);
      ...
      begin
        ...
        begin
          NAT_BC_CRED := StrToNatBcCred(sqlConsulta.FieldByName('COD_BASE_CALCULO')
            .AsString);
...
begin
  ...
  sqlConsulta.SQL.Clear;
  sqlConsulta.SQL.Add
    ('select sum(SP.VALOR_BASE) VALOR_BASE, sum(SP.valor_pis) VALOR_PIS, ');
  ...
  begin
    Posicao := Posicao + 1;
    if Mem400.Locate('CST_PIS', sqlConsulta.FieldByName('CST_PIS').AsString,
      [loCaseInsensitive]) then
      Mem400.Edit
    else
      Mem400.Append;
    Mem400CST_Pis.AsString := sqlConsulta.FieldByName('CST_PIS').AsString;
    Mem400Valor.AsFloat := Mem400Valor.AsFloat + sqlConsulta.FieldByName
      ('VALOR_BASE').AsFloat;
...
begin
  ...
  sqlConsulta.SQL.Add
    ('sum(SP.VALOR_COFINS) VALOR_COFINS, SP.CST_COFINS, SP.PERC_COFINS, SP.COD_BASE_CALCULO ');
  sqlConsulta.SQL.Add('from SPED_PISCOFINS_ENTRADA SP ');
  sqlConsulta.SQL.Add('WHERE SP.data_emissao between ' +
    QuotedStr(FormatDateTime('mm/dd/yyyy', DataInicial)) + ' AND ');
  sqlConsulta.SQL.Add(QuotedStr(FormatDateTime('mm/dd/yyyy', DataFinal)));
  ...
  sqlConsulta2.SQL.Clear;
  sqlConsulta2.SQL.Add
    (' select sum(SP.VALOR_COFINS) VALOR_COFINS, sum(SP.VALOR_BASE) VALOR_BASE, ');
  sqlConsulta2.SQL.Add(' iif (SP.CST_COFINS =  ' + QuotedStr('01') + ', ' +
    QuotedStr('01') + ', ' + QuotedStr('32') + ') as CST_COFINS, SP.ALIQ_COFINS ');
  sqlConsulta2.SQL.Add('from SPED_PISCOFINS_SAIDA SP ');
  sqlConsulta2.SQL.Add('WHERE SP.data_emissao between ' +
    QuotedStr(FormatDateTime('mm/dd/yyyy', DataInicial)) + ' AND ');
  sqlConsulta2.SQL.Add(QuotedStr(FormatDateTime('mm/dd/yyyy', DataFinal)));
  ...
  begin
    ...
    Mem500VlrBaseCofins.AsFloat := Mem500VlrBaseCofins.AsFloat +
      sqlConsulta.FieldByName('VALOR_BASE_COFINS').AsFloat;
    Mem500AliqCofins.AsFloat := sqlConsulta.FieldByName('PERC_COFINS').AsFloat;
    Mem500VlrCredito.AsFloat := Mem500VlrCredito.AsFloat + sqlConsulta.FieldByName
      ('VALOR_COFINS').AsFloat;
    Mem500VlrCreditoDisponivel.AsFloat := Mem500VlrCreditoDisponivel.AsFloat +
      sqlConsulta.FieldByName('VALOR_COFINS').AsFloat;
    ...
    begin
      if sqlConsulta2.Locate('CST_COFINS', '01', [loCaseInsensitive]) then
        Mem500VlrCreditoDescontado.AsFloat :=
          sqlConsulta2.FieldByName('VALOR_COFINS').AsFloat
      else
        Mem500VlrCreditoDescontado.AsFloat := 0;
      Mem500SaldoCredito.AsFloat := Mem500VlrCredito.AsFloat -
        Mem500VlrCreditoDescontado.AsFloat;
  ...
  begin
    ...
    begin
      ...
      sqlConsulta.Filtered := False;
      sqlConsulta.Filter := 'PERC_COFINS = ' +
        StringReplace(FormatFloat('0.00', Mem500AliqCofins.AsFloat), ',', '.',
        [rfReplaceAll]);
      sqlConsulta.Filtered := true;
      sqlConsulta.First;
      while not sqlConsulta.Eof do
...
begin
  ...
  sqlConsulta.SQL.Clear;
  sqlConsulta.SQL.Add
    ('select sum(SP.VALOR_BASE) VALOR_BASE, sum(SP.valor_cofins) VALOR_COFINS, ');
  ...
  begin
    Posicao := Posicao + 1;
    if Mem800.Locate('CST_COFINS', sqlConsulta.FieldByName('CST_COFINS').AsString,
      [loCaseInsensitive]) then
      Mem800.Edit
    else
      Mem800.Append;
    Mem800CST_Cofins.AsString := sqlConsulta.FieldByName('CST_COFINS').AsString;
    Mem800Valor.AsFloat := Mem800Valor.AsFloat + sqlConsulta.FieldByName
      ('VALOR_BASE').AsFloat;
...
begin
  ...
  begin
    ...
    sqlProdutoUNIDADEINVENTARIO.AsString :=
      sqlConsulta.FieldByName('UNIDA5DESCR').AsString;
